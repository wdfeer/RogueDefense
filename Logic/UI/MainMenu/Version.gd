...
func _ready():
	...
	_update_latest_version_visual()

func _update_latest_version_visual():
	var current_version = text
	if _is_version_greater(latest_version, current_version):
...
func _is_version_greater(v1: String, v2: String):
	if v1 == v2:
		return false
	
	var numbers1 = _split_version(v1)
	var numbers2 = _split_version(v2)
...
func _split_version(version: String) -> Array :
