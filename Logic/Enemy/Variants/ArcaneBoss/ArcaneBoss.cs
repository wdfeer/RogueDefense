...
namespace RogueDefense.Logic.Enemy.Variants.ArcaneBoss;
...
public partial class ArcaneBoss : Enemy
{
    ...
    {
        maxHp += 1000 * Game.GetStage() * NetworkManager.PlayerCount;
    }
    ...
    public int AliveNodes => nodes.Count(x => !IsInstanceValid(x) || x.Dead);
    ...
    [Export] PackedScene arcaneBulletScene;
    ...
    void Shoot()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            float spread = Mathf.Lerp(Mathf.DegToRad(30), Mathf.DegToRad(5), AliveNodes / 4f);
            bullet.velocity = GlobalPosition.DirectionTo(target).Rotated(spread * GD.Randf() * 2 - 1f) * (200 + i * 40);

            bullet.damage = damage / (AliveNodes + 2);
            bullet.lifespan = 3;
        }
    }
}