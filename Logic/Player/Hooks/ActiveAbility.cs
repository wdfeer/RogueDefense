...
{
    Button button;
    public ActiveAbility(Core.Player player, Button button) : base(player)
    {
        if (button == null)
            return;
        this.button = button;
    ...
    {
        Client.instance.SendMessage(MessageType.AbilityActivated, new AbilityActivatedMessage()
        {
            from = Client.myId,
            abilityTypeIndex = GetAbilityIndex()
        });
    }
